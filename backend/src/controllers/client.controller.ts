...
const getClientId = async (userId: number): Promise<number> => {
  const result = await query('SELECT idU_cl FROM client WHERE idU_cl = $1', [userId]);
  if (result.rows.length > 0) {
    return result.rows[0].idu_cl;
  }
...
export const getDependants = async (req: AuthRequest, res: Response) => {
  ...
  const result = await query('SELECT * FROM dependant WHERE id_U_CL = $1', [clientId]);
...
export const addDependant = async (req: AuthRequest, res: Response) => {
  ...
  const result = await query(
    'INSERT INTO dependant (name, date_of_birth, relationship, id_U_CL) VALUES ($1, $2, $3, $4) RETURNING *',
...
export const updateDependant = async (req: AuthRequest, res: Response) => {
  ...
  const { name, date_of_birth, relationship } = req.body;
  const result = await query(
    'UPDATE dependant SET name = $1, date_of_birth = $2, relationship = $3 WHERE id_dep = $4 AND id_U_CL = $5 RETURNING *',
...
export const deleteDependant = async (req: AuthRequest, res: Response) => {
  const clientId = await getClientId(req.userId!);
  const { id } = req.params;
  await query('DELETE FROM dependant WHERE id_dep = $1 AND id_U_CL = $2', [id, clientId]);
...
export const getMedicalInfo = async (req: AuthRequest, res: Response) => {
  ...
  const result = await query('SELECT * FROM medical_info WHERE id_dep = $1', [id]);
...
export const updateMedicalInfo = async (req: AuthRequest, res: Response) => {
  ...
  const existing = await query('SELECT id_dep FROM medical_info WHERE id_dep = $1', [id]);

  let result;
  if (existing.rows.length > 0) {
    result = await query(
      'UPDATE medical_info SET blood_type = $1, allergies = $2, medications = $3, conditions = $4 WHERE id_dep = $5 RETURNING *',
      [blood_type, allergies, medications, conditions, id]
    );
  } else {
    result = await query(
      'INSERT INTO medical_info (blood_type, allergies, medications, conditions, id_dep) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [blood_type, allergies, medications, conditions, id]
    );
  }
  res.json({ success: true, medicalInfo: result.rows[0] });
...
export const getAuthorizedPeople = async (req: AuthRequest, res: Response) => {
  const clientId = await getClientId(req.userId!);
  const result = await query('SELECT * FROM authorized_person WHERE id_U_CL = $1', [clientId]);
...
export const updateAuthorizedPerson = async (req: AuthRequest, res: Response) => {
  ...
  const result = await query(
    'UPDATE authorized_person SET name = $1, phone_number = $2, national_id = $3 WHERE id_AP = $4 RETURNING *',
...
export const addAuthorizedPerson = async (req: AuthRequest, res: Response) => {
  ...
  const result = await query(
    'INSERT INTO authorized_person (name, phone_number, national_id, id_U_CL) VALUES ($1, $2, $3, $4) RETURNING *',
...
export const removeAuthorizedPerson = async (req: AuthRequest, res: Response) => {
  const clientId = await getClientId(req.userId!);
  const { id } = req.params;
  await query('DELETE FROM authorized_person WHERE id_AP = $1 AND id_U_CL = $2', [id, clientId]);
